...
- `List`（对付顺序的好帮手）: 存储的元素是有序的、可重复的，支持下标访问。
- `Set`（注重独一无二的性质）: 存储的元素不可重复的，主要用于去重。
- `Queue`（实现排队功能的叫号机）: 按特定的排队规则来确定先后顺序，存储的元素是有序的、可重复的，用于任务调度和生产者消费者模型。
- `Map`（用 key 来搜索的专家）: 使用键值对（key-value）存储，用于快速映射和查找。类似于数学上的函数 y=f(x)，"x" 代表 key，"y" 代表 value，key 无序、不可重复，value 无序、可重复，每个键最多映射到一个值。注意，这里的“无序”指的是 `HashMap` 这类实现——键值对之间没有显式的关联顺序。`LinkedHashMap` 和 `TreeMap` 等实现则是有序的，它们通过额外的数据结构（双向链表或红黑树）来维护键值对的顺序。
...
选择集合的核心是根据业务场景决定：
如果是键值对结构选择 Map，如果需要去重选择 Set，如果需要有序且允许重复选择 List，如果是任务处理或队列场景选择 Queue，如果是高并发场景则选择 Concurrent 系列集合。
其中 List 中 ArrayList 适合查询多场景，LinkedList 适合频繁插入删除；Map 中 HashMap 最常用，ConcurrentHashMap 用于并发场景；Set 用于去重；Queue 用于任务调度和生产者消费者模型。
